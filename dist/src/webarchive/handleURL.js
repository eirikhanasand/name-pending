...
export default async function fetchURL(link, generated, fast) {
    try {
        ...
        if (!response)
            console.log("No secure HEAD reply from " + link);
        ...
        if (!response)
            console.log("No secure GET reply from " + link);
        if (response.status === 200)
            return link;
        response = await fetch(link.replace('https://', 'http://'), { method: 'HEAD' });
        if (!response)
            console.log(`No insecure HEAD reply from ${link}`);
        if (response.status === 200)
            return link;
        response = await fetch(link.replace('https://', 'http://'), { method: 'GET' });
        if (!response)
            console.log(`No insecure GET reply from ${link}`);
        if (response.status === 200)
            return link;
        if (!response)
            return console.log(`No reply from ${link}`);
        if (generated)
            return;
        if (response.status)
            return link; // Returning on error to archive error pages
    }
    catch (e) {
        if (e.cause.code) {
            switch (e.cause.code) {
...
export async function archiveWorkingURLs(embed, links, stats) {
    ...
    await Promise.allSettled(links.map(async (url) => {
        ...
        if (response) {
            if (domain)
                stats.finished_domains++;
            else
                stats.finished_paths++;
        }
        else {
            if (domain)
                stats.domains_failed++;
