...
async function reply(message, service, status, reason, branch) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setTimestamp()
        .setAuthor({ name: `Author: ${message.user.username} · ${message.user.id}` })
        .addFields({ name: "Status", value: status, inline: true }, { name: "Reason", value: reason, inline: true }, { name: "Branch", value: branch, inline: true });
    await message.editReply({ embeds: [embed] });
}
/**
...
async function restartBeehive(message, reason, branch) {
    const embed = new EmbedBuilder()
        ...
        .setTimestamp()
        .setAuthor({ name: `Author: ${message.user.username} · ${message.user.id}` })
        .addFields({ name: "Status", value: "Starting...", inline: true }, { name: "Reason", value: reason, inline: true }, { name: "Branch", value: branch, inline: true });
    await message.editReply({ embeds: [embed] });
    const restart = [
        'rm -rf frontend',
...
async function replyInvalid(message, service, reason, branch) {
    const embed = new EmbedBuilder()
        ...
        .setAuthor({ name: `Author: ${message.user.username} · ${message.user.id}` })
        .setTimestamp()
        .addFields({ name: serviceExists(service) ? "Service" : "Invalid service", value: service ? service : "undefined", inline: true }, { name: reason ? "Reason" : "Invalid reason", value: reason ? reason : "undefined", inline: true }, { name: "Branch", value: branch, inline: true });
    await message.editReply({ embeds: [embed] });
}
