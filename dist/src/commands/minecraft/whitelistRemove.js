...
async function post(message, name) {
    let content = "";
    await Promise.all(config.minecraft_servers.map(async (server) => {
        ...
        switch (response.status) {
            case 304:
                ...
                break;
            case 418:
                content = `Removed ${name} from the whitelist.`;
                break;
            case 404:
                content = `There is no Minecraft account named ${name}. Please try again.`;
                break;
            default:
