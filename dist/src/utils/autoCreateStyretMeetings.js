...
dotenv.config();
const { DISCORD_SAKER_TIL_STYREMOTER_CHANNEL_ID, DISCORD_STYRET_INNKALLING_CHANNEL_ID } = process.env;
...
export default async function autoCreateStyretMeetings(client) {
    ...
    const styremote = await client.channels.fetch(DISCORD_STYRET_INNKALLING_CHANNEL_ID);
    if (!channel) {
