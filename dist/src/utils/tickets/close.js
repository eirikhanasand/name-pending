...
export async function handleCloseSelectedTicket(interaction) {
    const guild = interaction.guild;
    if (!guild) {
        return await interaction.reply({
    ...
    try {
        ...
        const archiveCategory = guild.channels.cache.find(c => c instanceof CategoryChannel && c.name === "archived-tickets");
        if (!archiveCategory) {
            return await interaction.reply({
