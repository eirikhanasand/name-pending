...
export default function log(message: ChatInputCommandInteraction<CacheType>, content: string) {
    ...
    if (!guild) {
        return console.error('Message does not contain a valid guild.')
    }

    if (!message.member) {
        return console.error('Invalid user, user has likely been deleted or modified in an unrecognizable way.')
    }
